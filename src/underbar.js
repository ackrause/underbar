...
(function() {
  ...
  _.map = function(array, iterator) {
    ...
    var saveIteratorResults = function(value, key, collection) {
      result.push(iterator(value, key, collection));
    };

    _.each(array, saveIteratorResults);
  ...
  _.sortBy = function(collection, iterator) {
    ...
    var temp = _.map(collection, function(value, index) {
      return {
        value: value,
        index: index,
        comparator: iterator(value, index, collection)
      };
    }).sort(function(a, b) {
      var x = a.comparator;
      var y = b.comparator;
      if (x !== y) {
        ...
        return a.index - b.index; // try and keep things in same order
      }
    });

    return _.pluck(temp, 'value');
  };
  ...
  _.intersection = function() {
    ...
    for (var i = 0; i < arguments[0].length; i++) {
      var testVal = arguments[0][i];
      if (_.every(arguments, function(arr) { return _.contains(arr, testVal); })) {
        result.push(testVal);
      }
    }
  ...
  _.difference = function(array) {
    var result = [];
    var testArrays = Array.prototype.slice.call(arguments, 1);

    for (var i = 0; i < array.length; i++) {
      var testVal = array[i];
      if (!_.some(testArrays, function(arr) { return _.contains(arr, testVal); })) {
        result.push(testVal);
      }
    }
